...
namespace versia1
{
    public partial class dobav_test_leg : Form
    {
        ...
        string razd, tema, n_test, vopr;
        int id_yrov, id_temi, min_ballov, id_test, flag = 0;
        int id_query = 0;
        string section;
        string action_type = "test";

        #region Обработчики событий
        private void Stop_Click(object sender, EventArgs e) // Окончание добавления теста
        {
            if (Vopros.Text == "")
            {
        ...
        private void Vvod_cled_vop_Click(object sender, EventArgs e)
        {
            dobavleni();
        }        
        ...
        {
            dobav_test_leg.ActiveForm.Hide();
        ...
        private void list_test_SelectedIndexChanged(object sender, EventArgs e) // Заполнение DataGridView вопросами
        {
        ...
        private void dataGrid_view_questions_SelectionChanged(object sender, EventArgs e) // Переход к заполнению DataGridView ответами
        {
        ...
        int id_quest;
        private void dataGrid_view_questions_CellContentClick(object sender, DataGridViewCellEventArgs e) // Переход к заполнению DataGridView ответами
        {
            if (e.RowIndex >= 0)
            {
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e) // Вызов необходимых функции заполнения комбобоксов в зависимости от вкладки
        {
        ...
        private void save_modified_answer_Click(object sender, EventArgs e) // Сохранение изменённого ответа
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("Вы пытаетесь обновить верность ответа. Пожалуйста, нажмите 'Сохранить верность'.", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
        ...
        private void change_sections_SelectedIndexChanged(object sender, EventArgs e) // Переход к заполнению комбобокса тестами
        {
            section = change_sections.Text;
            combobox_test(section, "Лёгкий", list_test);
            changing_question.Clear();
            changing_answers.Clear();
            list_test.Text = "";
        }

        private void save_change_allegiance_Click(object sender, EventArgs e) // Обновление верности ответа
        {
            if (changing_answers.Text == "")
            {
                MessageBox.Show("Выберите вопрос.", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            else
            {
                if (changing_answers.Text == "1" || changing_answers.Text == "0")
                {
                    bool correct_answer_exists = Existence_correct_answer(id_quest);
                    int correct = Convert.ToInt32(changing_answers.Text);
                    if (correct == 1 && correct_answer_exists)
                    {
                        MessageBox.Show("Верный ответ для этого вопроса уже существует.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }

                    string modified_correct = changing_answers.Text;
                    string query = "UPDATE otveti SET vernost = @modified_correct WHERE id_otvet = @id_answer";
                    try
                    {
                        using (MySqlConnection connection = new MySqlConnection(connectionString))
                        {
                            ...
                            {
                                command.Parameters.AddWithValue("@id_answer", id_answer);
                                command.Parameters.AddWithValue("@modified_correct", modified_correct);
                                command.ExecuteNonQuery();
                            }
                        }
                        MessageBox.Show("Верность ответа успешно обновлёна", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        search_answers(id_quest, dataGridView_answers);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show("Ошибка при обновлении верности ответа: " + ex.Message);
                    }
                }
                else
                {
                    MessageBox.Show("Вы пытаетесь обновить ответ. Пожалуйста, нажмите 'Сохранить ответ'.", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private void adding_or_removing_sections_SelectedIndexChanged(object sender, EventArgs e) // Переход к заполнению комбобокса тестами
        {
            section = adding_or_removing_sections.Text;
            combobox_test(section, "Лёгкий", adding_or_removing_themes);
        ...
        private void add_or_delete_correct_KeyPress(object sender, KeyPressEventArgs e)
        {
            char digit = e.KeyChar;
            if (digit != 8 && digit != 48 && digit != 49)
                e.Handled = true;
        }
        ...
        private void add_or_delete_answer_detgrview_CellClick(object sender, DataGridViewCellEventArgs e) // Вывод ответа и верности в текстовые поля
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (cellValue != null)
                {
                    ...
                    string correct = add_or_delete_answer_detgrview.Rows[e.RowIndex].Cells["Correct_add_del"].Value.ToString();
                    ...
                    add_or_delete_correct.Text = correct;
        ...
        private void add_or_delete_answer_detgrview_SelectionChanged(object sender, EventArgs e) // Вывод ответа и верности в текстовые поля
        {
                ...
                if (value != null)
                {
                    if (add_or_delete_answer_detgrview.SelectedRows.Count > 0)
                    {
                        ...
                        if (index >= 0 && index < add_or_delete_answer_detgrview.Rows.Count)
                        {
                            ...
                            string correct = add_or_delete_answer_detgrview.Rows[index].Cells["Correct_add_del"].Value.ToString();
        ...
        private void add_question_Click(object sender, EventArgs e) // Переход к добавлению вопроса и обновлению таблицы
        {
        ...
        private void add_answer_Click(object sender, EventArgs e) // Добавление ответа
        {
            ...
            else
            {
                string question = adding_or_removing_questions.Text;
                int id_question = Convert.ToInt32(add_or_delete_question_detgrview.CurrentRow.Cells["id_question_add_or_delete"].Value);
                int answer_count = Number_answers(id_quest);
                if (answer_count >= 10)
                {
                    MessageBox.Show("Достигнуто максимальное количество ответов для этого вопроса (10).", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                bool correct_answer_exists = Existence_correct_answer(id_quest);
                int correct = Convert.ToInt32(add_or_delete_correct.Text);
                if (correct == 1 && correct_answer_exists)
                {
                    MessageBox.Show("Верный ответ для этого вопроса уже существует.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
        ...
        {
            string answer_text = response_field.Text;
            var correct_t = Correct.Text == "1";
            if (Correct.Text == "")
            {
                MessageBox.Show("Введите верность ответа.");
            }
            else if (response_field.Text == "")
            {
                MessageBox.Show("Введите ответ.");
            }
            else
            {
                if (list_answers.Items.Count < 10)
                {
                    if (flag == 1 && Correct.Text == "1")
                    {
                        MessageBox.Show("Верный ответ уже существует");
                    }
                    else
                    {
                ...
                {
                    MessageBox.Show("Максимальное количество ответов - 10.");
        ...
        private void Correct_KeyPress(object sender, KeyPressEventArgs e)
        {
            char digit = e.KeyChar;
            if (digit != 8 && digit != 48 && digit != 49)
                e.Handled = true;
        ...
        #region Функции
        private void dobavleni() // Добавление вопроса с ответами
        {
            flag = 0;
        ...
        private bool test_exists(string test_name) // Проверка наличия теста
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                connection.Open();
        ...
        private int Number_answers(int id_quest) 
        {
            int count = 0;
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                connection.Open();
                string query = "SELECT COUNT(*) FROM otveti WHERE id_vopr = @id_quest;";
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@id_quest", id_quest);
        ...
        private bool Existence_correct_answer(int id_quest)
        {
            bool exists = false;
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                connection.Open();
                string query = "SELECT EXISTS(SELECT 1 FROM otveti WHERE id_vopr = @id_quest AND vernost = 1);";
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@id_quest", id_quest);
        ...
        private int f_id_question(string question)
        {
        ...
        private int f_id_answer(string answer)
        {
        ...
        private int f_id_temit(string tema)
        {
