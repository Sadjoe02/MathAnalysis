...
namespace versia1
{
    public partial class adding_complex_tests : Form
    {
        ...
        public adding_complex_tests()
        {
            InitializeComponent();
            combobox_sections(razdel);
            id_yrov = 3;
            this.Size = new Size(1046, 650);
        }
        string action_type = "difficult_test";
        string razd, tema, n_test, vopr, otvet, section;
        int id_yrov, id_temi, min_ballov, id_test, id_query, id_quest, id_answer;

        #region Обработчики событий 
        private void Vvod_cled_vop_Click(object sender, EventArgs e) // Ввод следующего вопроса
        {
            dobavleni();
        }

        private void Stop_Click(object sender, EventArgs e) // Окончание ввода сложного теста
        {
            if (task_field.Image == null)
            {
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e) // Изменение размера окна в зависимости от выбранной вкладки
        {
        ...
        private void change_sections_SelectedIndexChanged(object sender, EventArgs e) // Переход к заполнению комбобокса названиями сложных тестов по соответствующему разделу
        {
            section = change_sections.Text;
            combobox_test(section, "Сложный", list_test);
            task_display.Image = null;
            changing_answers.Clear();
            list_test.Text = "";
        }

        private void list_test_SelectedIndexChanged(object sender, EventArgs e) // Заполнение таблицы вопросами в соответствии с тестом
        {
        ...
        private void add_task_Click(object sender, EventArgs e) // Добавление изображения с заданием
        {
            string startupPath = Application.StartupPath; 
            string tasksPath = Path.Combine(startupPath, "tasks_for_difficult_test"); 

            openFileDialog1.InitialDirectory = tasksPath;
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                task_field.Image = Image.FromFile(selectedFilePath);
            }
        }

        private void button1_Click(object sender, EventArgs e) // Переход к редактору формул
        {
        ...
        private void dataGrid_view_questions_SelectionChanged(object sender, EventArgs e) // Вывод ответов в соответвии с вопросом
        {
            if (dataGrid_view_questions.CurrentRow != null)
            {
                object value = dataGrid_view_questions.CurrentRow.Cells["Question"].Value;
                if (value != null && value is Image)  
                {
                    Image questionImage = (Image)value;  
                    task_display.Image = questionImage; 
                    ...
                    if (dataGrid_view_questions.SelectedRows.Count > 0)
                    {
                        ...
                        {
                            int id_quest = Convert.ToInt32(dataGrid_view_questions.Rows[index].Cells["id_question"].Value);
        ...
        private void button2_Click(object sender, EventArgs e) // Открытие редактора формул
        {
            formula_editor Myformula_editor = new formula_editor
            {
                Action_Type = action_type,
        ...
        private void dataGrid_view_questions_CellClick(object sender, DataGridViewCellEventArgs e) // Поиск и заполнение таблицы ответами 
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (cellValue != null)
                {
                        int id_quest = Convert.ToInt32(dataGrid_view_questions.Rows[e.RowIndex].Cells["id_question"].Value);
        ...
        private void change_task_Click(object sender, EventArgs e) // Загрузка в поле изображения
        {
            task_display.Image = null;
            string startupPath = Application.StartupPath; 
            string tasksPath = Path.Combine(startupPath, "tasks_for_difficult_test"); 
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                task_display.Image = Image.FromFile(selectedFilePath);
            }

        }

        private void save_modified_question_Click(object sender, EventArgs e)  // Замена существующего изображения
        {
            if (task_display.Image == null)
            {
                MessageBox.Show("Вы не выбрали задание.", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                int id_quest = Convert.ToInt32(dataGrid_view_questions.CurrentRow.Cells["id_question"].Value);
                byte[] imageBytes = Convert_Image_To_Byte_Array(task_display);
                if (imageBytes == null)
                {
                    MessageBox.Show("Ошибка при конвертации изображения.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                string query = "UPDATE voprosi SET question_for_difficult_test = @modified_question WHERE id_vopr = @id_quest";
                try
                {
                    using (MySqlConnection connection = new MySqlConnection(connectionString))
                    {
                        ...
                        using (MySqlCommand command = new MySqlCommand(query, connection))
                        {
                            ...
                            command.Parameters.Add("@modified_question", MySqlDbType.Blob).Value = imageBytes;  
                            command.ExecuteNonQuery();
                        }
                    }
                    MessageBox.Show("Изображение вопроса успешно обновлено", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    string selected_test = list_test.SelectedItem.ToString();
                    int id_test = search_id_test(selected_test);
                    filling_questions(id_test, dataGrid_view_questions);
                }
                ...
                {
                    MessageBox.Show("Ошибка при обновлении изображения вопроса: " + ex.Message);
                }
            }
        }

        private void dataGridView_answers_SelectionChanged(object sender, EventArgs e) // Вывода ответа в поле
        {
        ...
        private void dataGridView_answers_CellClick(object sender, DataGridViewCellEventArgs e) // Вывод ответа в поле
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
        ...
        private void save_modified_answer_Click(object sender, EventArgs e) // Сохранение изменённого вопроса
        {
        ...
        private void adding_or_removing_sections_SelectedIndexChanged(object sender, EventArgs e) // Переход к заполнению комбобокса названиями сложных тестов по соответствующему разделу
        {
            section = adding_or_removing_sections.Text;
            combobox_test(section, "Сложный", adding_or_removing_themes);
            task_display.Image = null;
            changing_answers.Clear();
            adding_or_removing_themes.Text = "";
        }

        private void adding_or_removing_themes_SelectedIndexChanged(object sender, EventArgs e) // Заполнение таблицы вопросами в соответствии с тестом
        {
            string selected_test = adding_or_removing_themes.SelectedItem.ToString();
            int id_test = search_id_test(selected_test);
            filling_questions(id_test, dataGridView_add_or_delete_quest);
            task_display.Image = null;
            adding_or_removing_answers.Clear();
        }

        private void button3_Click(object sender, EventArgs e) // Переход к редактору формул
        {
            formula_editor Myformula_editor = new formula_editor
            {
                Action_Type = action_type,
        ...
        private void add_question_Click(object sender, EventArgs e) // Сохранение нового вопроса
        {
            byte[] imageBytes = Convert_Image_To_Byte_Array(field_task_delete_or_add);
            if (imageBytes == null)
            {
                MessageBox.Show("Ошибка при конвертации изображения.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            n_test = adding_or_removing_themes.Text;
            id_test = search_id_test(n_test);
            int currentCount = count_questions(id_test);
            save_query(imageBytes, id_test);
            filling_questions(id_test, dataGridView_add_or_delete_quest);
        }

        private void dataGridView_add_or_delete_quest_SelectionChanged(object sender, EventArgs e) // Заполнение таблицы ответами в соответствии с вопросом
        {
            if (dataGridView_add_or_delete_quest.CurrentRow != null)
            {
                object value = dataGridView_add_or_delete_quest.CurrentRow.Cells["Question_add_or_delete"].Value;
                if (value != null && value is Image)  
                {
                    Image questionImage = (Image)value;  
                    field_task_delete_or_add.Image = questionImage; 

                    if (dataGridView_add_or_delete_quest.SelectedRows.Count > 0)
                    {
                        int index = dataGridView_add_or_delete_quest.SelectedRows[0].Index;

                        if (index >= 0 && index < dataGridView_add_or_delete_quest.Rows.Count)
                        {
                            int id_quest = Convert.ToInt32(dataGridView_add_or_delete_quest.Rows[index].Cells["id_question_add_or_delete"].Value);
                ...
                {
                    field_task_delete_or_add.Image = null; 
                }
            }

        }

        private void dataGridView_add_or_delete_quest_CellClick(object sender, DataGridViewCellEventArgs e) // Заполнение таблицы ответами в соответствии с вопросом
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                var cellValue = dataGridView_add_or_delete_quest.Rows[e.RowIndex].Cells[e.ColumnIndex].Value;
                if (cellValue != null)
                {
                    int id_quest = Convert.ToInt32(dataGridView_add_or_delete_quest.Rows[e.RowIndex].Cells["id_question_add_or_delete"].Value);
        ...
        private void delete_question_Click(object sender, EventArgs e) // Удаление вопроса
        {
            ...
            else
            {
                ...
                {
                    int id_question = Convert.ToInt32(dataGridView_add_or_delete_quest.CurrentRow.Cells["id_question_add_or_delete"].Value);
        ...
        private void add_answer_Click(object sender, EventArgs e) // Добавление ответа
        {
            ...
            {
                MessageBox.Show("Вы не ввели ответ.", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                int id_question = Convert.ToInt32(dataGridView_add_or_delete_quest.CurrentRow.Cells["id_question_add_or_delete"].Value);
                ...
                string query = "INSERT INTO otveti (id_vopr, otvet, vernost) VALUES (@id_quest, @answer, @correct);";
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    connection.Open();
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@id_quest", id_question);
                        command.Parameters.AddWithValue("@answer", answer);
                        command.Parameters.AddWithValue("@correct", 1);
                        command.ExecuteNonQuery();
                    }
                }
                search_answers(id_question, add_or_delete_answer_detgrview);
        ...
        private void add_or_delete_answer_detgrview_SelectionChanged(object sender, EventArgs e) // Вывод в поле ответа
        {
            if (add_or_delete_answer_detgrview.CurrentRow != null && add_or_delete_answer_detgrview.CurrentRow.Cells["Answers_add_del"] != null)
            {
                ...
                if (value != null)
                {
                    if (add_or_delete_answer_detgrview.SelectedRows.Count > 0)
                    {
                        ...
                        {
                            string answers = add_or_delete_answer_detgrview.Rows[index].Cells["Answers_add_del"].Value.ToString();

                            adding_or_removing_answers.Text = answers;
        ...
        private void add_or_delete_answer_detgrview_CellClick(object sender, DataGridViewCellEventArgs e) // Вывод в поле ответа
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (cellValue != null)
                {
                    adding_or_removing_answers.Text = cellValue.ToString();
                    id_answer = Convert.ToInt32(add_or_delete_answer_detgrview.Rows[e.RowIndex].Cells["Id_answer_add_del"].Value.ToString());
                }
            }
        }

        private void button4_Click(object sender, EventArgs e) // Загрузка в поле изображения
        {
            field_task_delete_or_add.Image = null;
            string startupPath = Application.StartupPath;
            string tasksPath = Path.Combine(startupPath, "tasks_for_difficult_test");
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                field_task_delete_or_add.Image = Image.FromFile(selectedFilePath);
            }
        ...
        {
            adding_complex_tests.ActiveForm.Hide();
        ...
        private void razdel_SelectedIndexChanged(object sender, EventArgs e)
        {
            razd = razdel.Text;
        ...
        #region Функции
        private void dobavleni() // Добавление задания
        {
            ...
            else
            {
                if (task_field.Image == null)
                {
                    MessageBox.Show("Вы не добавили задание.", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                else if (Otvet.Text == "")
                {
                    MessageBox.Show("Вы не ввел ответ.", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                else
                {
                    byte[] imageBytes = Convert_Image_To_Byte_Array(task_field);
                    if (imageBytes == null)
                    {
                        MessageBox.Show("Ошибка при конвертации изображения.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    ...
                    if (id_test == 0)
                    {
                        MessageBox.Show("Вы не нажали ОК при добавлении наименования теста.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    save_query(imageBytes, id_test);
                    otvet = Otvet.Text;
                    save_answer(id_query, otvet, true);
                    task_field.Image = null;
                    Otvet.Clear();
                }
            }
        ...
        private byte[] Convert_Image_To_Byte_Array(PictureBox pictureBox_name) // Конвертация изображения
        {
            if (pictureBox_name.Image != null)
            {
                using (MemoryStream ms = new MemoryStream())
                {
                    pictureBox_name.Image.Save(ms, System.Drawing.Imaging.ImageFormat.Png);
                    return ms.ToArray(); // Возвращает массив байтов
                }
            }
            return null;
        }
        ...
        private int count_questions(int id_test) // Подсчет заданий
        {
            string query = "SELECT COUNT(*) FROM voprosi_testov WHERE id_tests = @id_test;";
        ...
        private int save_query(byte[] imageBytes, int id_test) // Сохранение вопроса
        {
            string query = "INSERT INTO voprosi (vopros, question_for_difficult_test) VALUES ('-', @image); SELECT LAST_INSERT_ID();";
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                connection.Open();
                using (MySqlCommand command = new MySqlCommand(query, connection))
        ...
        private bool test_exists(string test_name) // Проверка на существование теста
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                connection.Open();
        ...
        private void search_answers(int id_question, DataGridView dataGridView_name)
        {
        ...
        private void filling_questions(int id_test, DataGridView dataGridView_name)
        {
            dataGridView_name.Rows.Clear();
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                string query = @"
        SELECT voprosi.id_vopr, voprosi.question_for_difficult_test  
        FROM voprosi_testov
                ...
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            if (!(reader["question_for_difficult_test"] is DBNull))
                            {
                                byte[] imageBytes = (byte[])reader["question_for_difficult_test"];
                                Image image;
                                using (var ms = new MemoryStream(imageBytes))
                                {
                                    image = Image.FromStream(ms);
                                }
                                dataGridView_name.Rows.Add(image, id_vopr); 
                            }
                            else
                            {
                                dataGridView_name.Rows.Add(new Bitmap(1, 1), id_vopr);
                            }
                        }
                    }
                }
